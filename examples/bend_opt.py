...
source = ModeSource(design=design, start=(0.5*µm, 2.7*µm), end=(0.5*µm, 4.1*µm), wavelength=WL, signal=signal)
source.show()
design.add(source)
#monitor = PowerMonitor(position=(3*µm, 0.5*µm), size=(1.4*µm, 0), axis="y")
#design.add(monitor)
design.show()
