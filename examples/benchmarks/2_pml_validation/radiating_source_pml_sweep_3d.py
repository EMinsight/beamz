"""3D radiating-source absorber benchmark for CPML versus graded absorber.

This is the 3D analogue of ``radiating_source_pml_sweep.py``. It launches a
compact Gaussian pulse in homogeneous vacuum and measures the residual field
energy left in the non-PML interior after the outgoing radiation should have
left the domain.
"""
...
def _run_case(cfg: SweepConfig, formulation: str, pml_thickness_wl: float):
    ...
    sim = Simulation(
        ...
        sources=[
            GaussianSource(
                position=(0.5 * size_m, 0.5 * size_m, 0.5 * size_m),
    ...
    energies = np.mean(np.square(ez_hist), axis=(1, 2, 3))

    start_ix = int(round(pml_m / dx))
    end_ix = int(round((size_m - pml_m) / dx))
    start_iy = int(round(pml_m / dx))
    end_iy = int(round((size_m - pml_m) / dx))
    start_iz = int(round(pml_m / dx))
    end_iz = int(round((size_m - pml_m) / dx))
    interior_hist = ez_hist[:, start_iz:end_iz, start_iy:end_iy, start_ix:end_ix]
    interior_energies = np.mean(np.square(interior_hist), axis=(1, 2, 3))
...
def _plot_results(results: list[CaseResult], out_dir: Path):
    ...
    ax.set_title("3D Radiating-Source Residual Energy vs PML Thickness")
    ax.set_xlabel("PML thickness (wavelengths)")
    ax.set_ylabel("Late / peak interior energy (dB)")
    ax.grid(alpha=0.3)
    ax.legend(frameon=False)
    fig.tight_layout()
    out_path = out_dir / "radiating_source_pml_sweep_3d.png"
...
def main() -> None:
    ...
    for formulation in cfg.pml_formulations:
        for thickness_wl in cfg.pml_thicknesses_wl:
            ...
            case, _debug = _run_case(cfg, formulation, thickness_wl)
            results.append(case)
