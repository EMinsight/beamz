...
class TestPMLAbsorption:
    ...
    def test_cpml_step_path_runs_without_nan(self, vacuum_domain_small):
        design = vacuum_domain_small["design"]
        wavelength = vacuum_domain_small["wavelength"]
        dx = vacuum_domain_small["dx"]
        dt = vacuum_domain_small["dt"]
        ...
        for _ in range(min(25, sim.num_steps)):
            assert sim.step()

        assert np.isfinite(np.asarray(sim.fields.Ez)).all()
        assert np.isfinite(np.asarray(sim.fields.Hx)).all()
        assert np.isfinite(np.asarray(sim.fields.Hy)).all()

    def test_cpml_3d_step_path_runs_without_nan(self):
        wavelength = 1.55 * um
        dx, dt = calc_optimal_fdtd_params(
            wavelength, 1.0, dims=3, safety_factor=0.95, points_per_wavelength=8
        )
        domain = 3.0 * wavelength
        steps = 8
        ...
        center = tuple(dim // 2 for dim in sim.fields.Ez.shape)
        sim.fields.Ez = sim.fields.Ez.at[center].set(1.0)

        for _ in range(min(steps, sim.num_steps)):
            assert sim.step()

        for component in ("Ex", "Ey", "Ez", "Hx", "Hy", "Hz"):
            arr = np.asarray(getattr(sim.fields, component))
            assert np.isfinite(arr).all()
        assert float(np.max(np.abs(np.asarray(sim.fields.Ez)))) > 0.0
