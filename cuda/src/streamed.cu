...
__device__ __forceinline__ float Read2D(const BeamzBuffer& value, int y,
                                        int x) {
  if (value.rank == 0) return static_cast<const float*>(value.data)[0];
  const int iy = value.dims[0] == 1 ? 0 : y;
  const int ix = value.dims[1] == 1 ? 0 : x;
  return static_cast<const float*>(value.data)[
      iy * static_cast<int>(value.dims[1]) + ix];
}

__device__ __forceinline__ void Write2D(const BeamzBuffer& value, int y, int x,
                                        float result) {
  static_cast<float*>(value.data)[y * static_cast<int>(value.dims[1]) + x] =
      result;
}

template <int MetricKind>
__device__ __forceinline__ float BoundaryDifference2D(
    const BeamzBuffer& value, int axis, int y, int x, int edge_mask,
    const BeamzLaunch& launch) {
  const int coordinate = axis == 1 ? y : x;
  const int size = static_cast<int>(value.dims[axis == 1 ? 0 : 1]);
  const int low_bit = axis == 1 ? 2 : 4;
  const float inv_dx = MetricScale<MetricKind>(launch, axis, coordinate);
  if (coordinate == 0) {
    return edge_mask & (1 << low_bit) ? Read2D(value, y, x) * inv_dx : 0.0f;
  }
  if (coordinate == size) {
    const int last_y = axis == 1 ? size - 1 : y;
    const int last_x = axis == 2 ? size - 1 : x;
    return edge_mask & (1 << (low_bit + 1))
               ? -Read2D(value, last_y, last_x) * inv_dx
               : 0.0f;
  }
  return axis == 1
             ? (Read2D(value, y, x) - Read2D(value, y - 1, x)) * inv_dx
             : (Read2D(value, y, x) - Read2D(value, y, x - 1)) * inv_dx;
}
...
template <int MetricKind>
__global__ void UpdateTmH2d(BeamzLaunch launch) {
  const int x = blockIdx.x * blockDim.x + threadIdx.x;
  const int y = blockIdx.y * blockDim.y + threadIdx.y;
  const BeamzBuffer& ez = launch.inputs[5];
  const BeamzBuffer& hx = launch.outputs[0];
  if (y < hx.dims[0] && x < hx.dims[1]) {
    const bool constrained =
        (x == 0 && (launch.metallic_edges & (1 << 4))) ||
        (x == hx.dims[1] - 1 && (launch.metallic_edges & (1 << 5)));
    const float derivative =
        (Read2D(ez, y + 1, x) - Read2D(ez, y, x)) *
        MetricScale<MetricKind>(launch, 1, y);
    Write2D(hx, y, x,
            constrained ? 0.0f
                        : Read2D(launch.inputs[6], y, x) *
                                  Read2D(launch.inputs[0], y, x) -
                              Read2D(launch.inputs[9], y, x) * derivative);
  }
  const BeamzBuffer& hy = launch.outputs[1];
  if (y < hy.dims[0] && x < hy.dims[1]) {
    const bool constrained =
        (y == 0 && (launch.metallic_edges & (1 << 2))) ||
        (y == hy.dims[0] - 1 && (launch.metallic_edges & (1 << 3)));
    const float derivative =
        (Read2D(ez, y, x + 1) - Read2D(ez, y, x)) *
        MetricScale<MetricKind>(launch, 2, x);
    Write2D(hy, y, x,
            constrained ? 0.0f
                        : Read2D(launch.inputs[7], y, x) *
                                  Read2D(launch.inputs[1], y, x) +
                              Read2D(launch.inputs[10], y, x) * derivative);
  }
}

template <int MetricKind>
__global__ void UpdateTmE2d(BeamzLaunch launch) {
  const int x = blockIdx.x * blockDim.x + threadIdx.x;
  const int y = blockIdx.y * blockDim.y + threadIdx.y;
  const BeamzBuffer& ez = launch.outputs[2];
  if (y >= ez.dims[0] || x >= ez.dims[1]) return;
  const bool constrained =
      (y == 0 && (launch.metallic_edges & (1 << 2))) ||
      (y == ez.dims[0] - 1 && (launch.metallic_edges & (1 << 3))) ||
      (x == 0 && (launch.metallic_edges & (1 << 4))) ||
      (x == ez.dims[1] - 1 && (launch.metallic_edges & (1 << 5)));
  const float curl = BoundaryDifference2D<MetricKind>(
                         launch.inputs[4], 2, y, x, launch.metallic_edges,
                         launch) -
                     BoundaryDifference2D<MetricKind>(
                         launch.inputs[3], 1, y, x, launch.metallic_edges,
                         launch);
  Write2D(ez, y, x,
          constrained
              ? 0.0f
              : Read2D(launch.inputs[8], y, x) *
                        Read2D(launch.inputs[2], y, x) +
                    Read2D(launch.inputs[11], y, x) * curl);
}

template <int MetricKind>
__global__ void UpdateTeH2d(BeamzLaunch launch) {
  const int x = blockIdx.x * blockDim.x + threadIdx.x;
  const int y = blockIdx.y * blockDim.y + threadIdx.y;
  const BeamzBuffer& hz = launch.outputs[2];
  if (y >= hz.dims[0] || x >= hz.dims[1]) return;
  const float d_ey_dx =
      (Read2D(launch.inputs[4], y, x + 1) -
       Read2D(launch.inputs[4], y, x)) *
      MetricScale<MetricKind>(launch, 2, x);
  const float d_ex_dy =
      (Read2D(launch.inputs[3], y + 1, x) -
       Read2D(launch.inputs[3], y, x)) *
      MetricScale<MetricKind>(launch, 1, y);
  Write2D(hz, y, x,
          Read2D(launch.inputs[8], y, x) *
                  Read2D(launch.inputs[2], y, x) -
              Read2D(launch.inputs[11], y, x) * (d_ey_dx - d_ex_dy));
}

template <int MetricKind>
__global__ void UpdateTeE2d(BeamzLaunch launch) {
  const int x = blockIdx.x * blockDim.x + threadIdx.x;
  const int y = blockIdx.y * blockDim.y + threadIdx.y;
  const BeamzBuffer& ex = launch.outputs[0];
  if (y < ex.dims[0] && x < ex.dims[1]) {
    const bool constrained =
        (y == 0 && (launch.metallic_edges & (1 << 2))) ||
        (y == ex.dims[0] - 1 && (launch.metallic_edges & (1 << 3)));
    const float derivative = BoundaryDifference2D<MetricKind>(
        launch.inputs[5], 1, y, x, launch.metallic_edges, launch);
    Write2D(ex, y, x,
            constrained ? 0.0f
                        : Read2D(launch.inputs[6], y, x) *
                                  Read2D(launch.inputs[0], y, x) +
  ...
  const BeamzBuffer& ey = launch.outputs[1];
  if (y < ey.dims[0] && x < ey.dims[1]) {
    const bool constrained =
        (x == 0 && (launch.metallic_edges & (1 << 4))) ||
        (x == ey.dims[1] - 1 && (launch.metallic_edges & (1 << 5)));
    const float derivative = BoundaryDifference2D<MetricKind>(
        launch.inputs[5], 2, y, x, launch.metallic_edges, launch);
    Write2D(ey, y, x,
            constrained ? 0.0f
                        : Read2D(launch.inputs[7], y, x) *
                                  Read2D(launch.inputs[1], y, x) -
...
template <int MetricKind>
void Launch2dUpdate(cudaStream_t stream, const BeamzLaunch& h_launch,
                    const BeamzLaunch& e_launch, int32_t polarization) {
  int64_t max_y = 0, max_x = 0;
  for (int component = 0; component < 3; ++component) {
    const BeamzBuffer& h_output = h_launch.outputs[component];
    const BeamzBuffer& e_output = e_launch.outputs[component];
    if (h_output.rank == 2) {
      max_y = h_output.dims[0] > max_y ? h_output.dims[0] : max_y;
      max_x = h_output.dims[1] > max_x ? h_output.dims[1] : max_x;
    }
    if (e_output.rank == 2) {
      max_y = e_output.dims[0] > max_y ? e_output.dims[0] : max_y;
      max_x = e_output.dims[1] > max_x ? e_output.dims[1] : max_x;
    }
  }
  const dim3 threads(32, 4, 1);
  const dim3 blocks((max_x + threads.x - 1) / threads.x,
                    (max_y + threads.y - 1) / threads.y, 1);
  if (polarization == 0) {
    UpdateTmH2d<MetricKind><<<blocks, threads, 0, stream>>>(h_launch);
    UpdateTmE2d<MetricKind><<<blocks, threads, 0, stream>>>(e_launch);
  } else {
    UpdateTeH2d<MetricKind><<<blocks, threads, 0, stream>>>(h_launch);
    UpdateTeE2d<MetricKind><<<blocks, threads, 0, stream>>>(e_launch);
  }
}
...
int BeamzLaunchStreamed2dSteps(void* raw_stream, const BeamzLaunch& h_launch,
                               const BeamzLaunch& e_launch,
                               int32_t polarization, int32_t nsteps) {
  if (nsteps < 1 || polarization < 0 || polarization > 1 ||
      h_launch.phase != 0 || e_launch.phase != 1 || h_launch.nterms != 0 ||
      e_launch.nterms != 0 || h_launch.metric_kind != e_launch.metric_kind ||
      h_launch.metric_kind < 0 || h_launch.metric_kind > 2) {
    return cudaErrorInvalidValue;
  }
  const int active_h[2] = {polarization == 0 ? 0 : 2,
                           polarization == 0 ? 1 : 2};
  const int active_e[2] = {polarization == 0 ? 2 : 0,
                           polarization == 0 ? 2 : 1};
  for (int index = 0; index < 2; ++index) {
    if (h_launch.outputs[active_h[index]].rank != 2 ||
        e_launch.outputs[active_e[index]].rank != 2 ||
        !FitsIntOffsets(h_launch.outputs[active_h[index]]) ||
        !FitsIntOffsets(e_launch.outputs[active_e[index]])) {
      return cudaErrorInvalidValue;
    }
  }
  for (int axis = 1; axis < 3; ++axis) {
    const BeamzBuffer& h_metric = h_launch.metrics[axis];
    const BeamzBuffer& e_metric = e_launch.metrics[axis];
    if (!FitsIntOffsets(h_metric) || !FitsIntOffsets(e_metric) ||
        (h_launch.metric_kind == 1 &&
         (h_metric.rank != 0 || e_metric.rank != 0)) ||
        (h_launch.metric_kind == 2 &&
         (h_metric.rank != 1 || e_metric.rank != 1 ||
          h_metric.dims[0] < 1 || e_metric.dims[0] < 1))) {
      return cudaErrorInvalidValue;
    }
  }

  auto stream = reinterpret_cast<cudaStream_t>(raw_stream);
  std::string graph_key = GraphKey(raw_stream, h_launch, e_launch, nsteps);
  graph_key.append(reinterpret_cast<const char*>(&polarization),
                   sizeof(polarization));
  GraphCache& cache = CachedGraphs();
  const bool cache_enabled = GraphCacheEnabled();
  if (cache_enabled) {
  ...
  auto launch_steps = [&]() {
    for (int32_t step = 0; step < nsteps; ++step) {
      if (h_launch.metric_kind == 0) {
        Launch2dUpdate<0>(stream, h_launch, e_launch, polarization);
      } else if (h_launch.metric_kind == 1) {
        Launch2dUpdate<1>(stream, h_launch, e_launch, polarization);
      } else {
        Launch2dUpdate<2>(stream, h_launch, e_launch, polarization);
      }
