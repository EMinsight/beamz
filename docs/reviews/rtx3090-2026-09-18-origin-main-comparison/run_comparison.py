...
out=root/'docs/reviews/rtx3090-2026-09-18-origin-main-comparison'
out.mkdir(exist_ok=True)
main=root/'.cache/perf/origin-main-comparison'
refroot=root/'.cache/perf/origin-main-comparison-states'
worker=root/'scripts/benchmark_cuda_revision.py'
variants={'main':(main,'012','auto'),'branch_default':(root,'012','auto'),'branch_layout':(root,'120','0')}
# Balance forward/reverse fresh-process order separately for each workload.
cases=[('narrow',(1024,256,64),'binary',['main','branch_default','branch_layout','branch_layout','branch_default','main']),('wide',(128,256,512),'binary',['main','branch_default','branch_default','main']),('irregular',(257,193,341),'smooth',['main','branch_default','branch_default','main'])]
...
for case,shape,material,order in cases:
 ...
 for index,name in enumerate(order):
  ...
  cmd=[sys.executable,str(worker),'--root',str(checkout),'--shape',*map(str,shape),'--material',material,'--output',str(out/(tag+'.json')),'--reference',str(refroot/case),'--axes',axes,'--fusion',fusion]
