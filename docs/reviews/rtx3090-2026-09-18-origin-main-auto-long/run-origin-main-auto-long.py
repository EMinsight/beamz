...
out=root/'docs/reviews/rtx3090-2026-09-18-origin-main-auto-long'
out.mkdir(exist_ok=True)
main=root/'.cache/perf/origin-main-comparison'
refroot=root/'.cache/perf/origin-main-auto-long-states'
...
cases=[('narrow_long',(1024,256,64),'binary',['main','branch_auto','branch_auto','main'])]
...
for case,shape,material,order in cases:
 ...
 for index,name in enumerate(order):
  ...
  cmd=[sys.executable,str(worker),'--root',str(checkout),'--steps','1025','--shape',*map(str,shape),'--material',material,'--output',str(out/(tag+'.json')),'--reference',str(refroot/case),'--axes',axes,'--fusion',fusion,'--tuning-cache',str(root/'docs/reviews/rtx3090-2026-09-18-autotune/cache')]
