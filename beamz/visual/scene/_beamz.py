...
def simulation_to_scene(simulation: Any) -> SceneSpec:
    ...
    objects = [
        _domain_box(design, bounds),
        *_structure_objects(design),
        *_monitor_objects(_simulation_monitors(simulation), design=design),
        *_source_objects(_simulation_sources(simulation), design=design),
...
def _design_bounds(design: Any) -> _Bounds:
    ...
    origin = _design_world_origin(design)
...
def _structure_objects(design: Any) -> list[Object3D]:
    ...
    for index, structure in enumerate(
        ...
        scene_object = _structure_object(
            structure, index=index, color=str(color), origin=origin
        )
...
def _structure_object(
    structure: Any, *, index: int, color: str, origin: tuple[float, float, float]
) -> Object3D | None:
    kind, geometry = _structure_geometry(structure, origin=origin)
    if kind is None:
        return None
    return Object3D(
        kind=kind,
...
def _structure_geometry(
    structure: Any, *, origin: tuple[float, float, float]
    ...
    if vertices:
        tapered_geometry = _tapered_structure_geometry(
            structure, z0=z0, depth=depth, origin=origin
        )
        if tapered_geometry is not None:
            return "triangle_mesh", tapered_geometry
        return "poly_extrusion", {
            "vertices": [
                [float(x) + origin[0], float(y) + origin[1]] for x, y, *_ in vertices
            ],
            "holes": [
                [[float(x) + origin[0], float(y) + origin[1]] for x, y, *_ in hole]
    ...
    if hasattr(structure, "position") and hasattr(structure, "radius"):
        ...
        return "sphere", {
            "center": [px + origin[0], py + origin[1], pz + origin[2]],
    ...
    if (
        ...
        return "box", {
            "center": [
                px + width / 2.0 + origin[0],
                py + height / 2.0 + origin[1],
                pz + depth / 2.0 + origin[2],
            ],
...
def _tapered_structure_geometry(
    structure: Any,
    *,
    z0: float,
    depth: float,
    ...
    if depth <= 0.0:
        return None
    if not bool(getattr(structure, "has_tapered_sidewalls", lambda: False)()):
        return None
    polygon_at_z = getattr(structure, "shapely_polygon_at_z", None)
    if polygon_at_z is None:
        return None

    bottom = polygon_at_z(z0)
    top = polygon_at_z(z0 + depth)
    if bottom is None or top is None:
        return None
    if not isinstance(bottom, ShapelyPolygon) or not isinstance(top, ShapelyPolygon):
        return None

    bottom_rings = _polygon_rings(bottom)
    top_rings = _polygon_rings(top)
    if len(bottom_rings) != len(top_rings):
        return None
    if any(len(br) != len(tr) for br, tr in zip(bottom_rings, top_rings)):
        return None

    positions: list[float] = []
    for triangle in _filtered_triangulation(bottom):
        coords = list(triangle.exterior.coords)[:-1]
        if len(coords) != 3:
            continue
        # Bottom cap faces outward in -z.
        for x, y in reversed(coords):
            positions.extend(
                (float(x) + origin[0], float(y) + origin[1], float(z0) + origin[2])
            )
    for triangle in _filtered_triangulation(top):
        coords = list(triangle.exterior.coords)[:-1]
        if len(coords) != 3:
            continue
        # Top cap faces outward in +z.
        for x, y in coords:
            positions.extend(
                (
                    float(x) + origin[0],
                    float(y) + origin[1],
                    float(z0 + depth) + origin[2],
                )
            )

    for bottom_ring, top_ring in zip(bottom_rings, top_rings):
        ring_len = len(bottom_ring)
        for idx in range(ring_len):
            next_idx = (idx + 1) % ring_len
            bx0, by0 = bottom_ring[idx]
            bx1, by1 = bottom_ring[next_idx]
            tx0, ty0 = top_ring[idx]
            tx1, ty1 = top_ring[next_idx]
            positions.extend(
                (
                    float(bx0) + origin[0],
                    float(by0) + origin[1],
    ...
    return {
        "positions": positions,
        "sidewall_angle": float(getattr(structure, "sidewall_angle", 0.0) or 0.0),
        "width_to_z": float(getattr(structure, "width_to_z", 0.0) or 0.0),
    }


def _polygon_rings(polygon: ShapelyPolygon) -> list[list[tuple[float, float]]]:
    rings: list[list[tuple[float, float]]] = [
        [(float(x), float(y)) for x, y, *_ in list(polygon.exterior.coords)[:-1]]
    ]
    for interior in polygon.interiors:
        rings.append([(float(x), float(y)) for x, y, *_ in list(interior.coords)[:-1]])
    return rings


def _filtered_triangulation(polygon: ShapelyPolygon) -> list[ShapelyPolygon]:
    triangles: list[ShapelyPolygon] = []
    for triangle in shapely_triangulate(polygon):
        if not isinstance(triangle, ShapelyPolygon) or triangle.is_empty:
            continue
...
def _monitor_objects(
    monitors: Iterable[Any], *, design: Any | None = None
) -> list[Object3D]:
    objects: list[Object3D] = []
    origin = _design_world_origin(design)
    for index, monitor in enumerate(monitors):
        ...
        if bool(getattr(monitor, "is_3d", False)):
            objects.append(_monitor_plane_object(monitor, label, origin=origin))
            continue
        ...
        objects.append(
            Object3D(
                ...
                geometry={
                    "points": [
                        [
                            float(start[0]) + origin[0],
                            float(start[1]) + origin[1],
                            origin[2],
                        ],
                        [
                            float(end[0]) + origin[0],
                            float(end[1]) + origin[1],
                            origin[2],
                        ],
                    ]
                },
...
def _monitor_plane_object(
    monitor: Any, label: str, *, origin: tuple[float, float, float]
) -> Object3D:
    plane_normal = str(getattr(monitor, "plane_normal", "z")).lower()
    center, size = _monitor_plane_geometry(monitor, plane_normal)
    center = _add_origin(center, origin)
    return Object3D(
        kind="plane",
        label=label,
        geometry={
...
def _monitor_plane_geometry(
    ...
    if snapped is not None:
        center = [float(v) for v in snapped.center]
        if plane_normal == "x":
            return center, [
                max(
                    float(snapped.axis_bounds("y")[1] - snapped.axis_bounds("y")[0]),
                    _MIN_SIZE,
                ),
                max(
        ...
        if plane_normal == "y":
            return center, [
                max(
                    float(snapped.axis_bounds("x")[1] - snapped.axis_bounds("x")[0]),
                    _MIN_SIZE,
                ),
                max(
                    float(snapped.axis_bounds("z")[1] - snapped.axis_bounds("z")[0]),
...
def _source_objects(
    sources: Iterable[Any], *, design: Any | None = None
) -> list[Object3D]:
    objects: list[Object3D] = []
    origin = _design_world_origin(design)
    for index, source in enumerate(sources):
        if (
            ...
            objects.append(_gaussian_source_object(source, index=index, origin=origin))
            continue
        if hasattr(source, "center") and hasattr(source, "width"):
            objects.extend(_mode_source_objects(source, index=index, origin=origin))
    return objects


def _gaussian_source_object(
    source: Any, *, index: int, origin: tuple[float, float, float]
) -> Object3D:
    center = _add_origin(_position3(getattr(source, "position")), origin)
...
def _mode_source_objects(
    source: Any, *, index: int, origin: tuple[float, float, float]
) -> list[Object3D]:
    snapped = getattr(source, "_snapped_region", None)
    if snapped is not None:
        center = _add_origin(tuple(float(v) for v in snapped.center), origin)
        ...
        center = _add_origin(_position3(getattr(source, "center")), origin)
...
def _design_world_origin(design: Any | None) -> tuple[float, float, float]:
    if design is None:
        return (0.0, 0.0, 0.0)
...
def _add_origin(
    coords: tuple[float, float, float] | list[float], origin: tuple[float, float, float]
) -> list[float]:
    return [
        float(coords[0]) + origin[0],
        float(coords[1]) + origin[1],
        float(coords[2]) + origin[2],
    ]
