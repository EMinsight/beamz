...
def test_sidewalled_polygon_adapts_to_triangle_mesh():
    structure = Polygon(
    ...
    mesh = next(obj for obj in scene.objects if obj.metadata.get("kind") == "structure")
    assert mesh.kind == "triangle_mesh"
    assert len(mesh.geometry["positions"]) > 0
