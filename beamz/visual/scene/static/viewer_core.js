...
function triangleMeshGeometryFromItem(item) {
...
function makeObjectOutline(spec, object, outlineColor, clippingPlanes) {
  ...
  if (
    spec.kind === "box"
    || spec.kind === "poly_extrusion"
    || spec.kind === "plane"
    || spec.kind === "triangle_mesh"
  ) {
...
function buildObject(spec, clippingPlanes, outlineColor, materialCache, orderIndex = 0) {
  ...
  switch (spec.kind) {
    ...
    case "triangle_mesh": {
      object = new THREE.Mesh(triangleMeshGeometryFromItem(geometry), material);
      break;
    }
    default:
      return null;
